...
function Workshop({id, selected, startTime, endTime, workshopTitle, major, level, workshopGiver, workshopDescription, onSelect}) {
	...
	return (
		<div className={`bg-white ${selected ? "border-blue-300 border-4" : "border-black border-2"} w-fit p-2 rounded-xl flex flex-col gap-2`} onClick={() => onSelect(id)}>
			<div>
			...
			<div className="flex flex-col gap-2">
				...
				<div>
					<div className="bg-blue-800 text-white px-1 rounded-md w-fit">{level}</div>
				</div>
				<div>{workshopGiver}</div>
				<div className="text-blue-400 ml-auto border-2 border-gray-400 hover:bg-blue-200 hover:text-white border-0 hover:cursor-pointer p-1 rounded-lg" onClick={handleSeeMoreClick}>
					See More
				</div>
				{dialogOpen && (
...
function SelectedWorkshops({selectedWorkshops}) {
	return (
		<div>
			...
			<ul>
				{selectedWorkshops.map((workshop, index) => (
					<li key={index}>
						Index: {index}, ID: {workshop.id}, Imaginary Event: {workshop.workshopTitle}
					</li>
				))}
			</ul>
		</div>
        ...
        <div className="flex flex-col gap-2">
          {workshops.map((workshop) => (
            <Workshop
              ...
              workshopGiver={workshop.workshopGiver}
...
export default function Page() {
	...
	const handleWorkshopSelect = (id) => {
		setWorkshops((prevWorkshops) => prevWorkshops.map((workshop) => (workshop.id === id ? {...workshop, selected: !workshop.selected} : workshop)))

		setSelectedWorkshops(workshops.filter((workshop) => workshop.selected))
	}

    const workshopsByRoom: Record<string, Workshop[]> = workshops.reduce((acc, workshop) => {
