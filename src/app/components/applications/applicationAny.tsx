...
const AnyApp: NextPage<AnyAppProps> = ({ tabs, AppType }) => {
    ...
    return (
        <Layout>
            ...
            <div className="h-screen bg-cover bg-center" style={{ backgroundImage: `url('/images/starfield-grad.jpg')` }}>
                <div className="h-screen flex items-center justify-center">
                    <div className="bg-white p-4 shadow-md rounded-lg h-2/3 w-2/3 flex flex-col">
                        <h1 className="text-2xl font-italics text-center">MIT Reality Hack 2024 -
                            <span className="text-2xl font-bold text-center mb-4"> {AppType} </span>Application
                        </h1>

                        <div className="flex justify-center space-x-4 mb-4">
                            <button
                                onClick={handlePreviousTab}
                                className={`text-lg cursor-pointer text-blue-500 hover:bg-blue-200 ${isOnFirstTab ? 'pointer-events-none text-gray-400' : ''}`}
                        ...
                        <div className="overflow-x-auto flex flex-row justify-center">
                            <Tabs value={selectedTab} onChange={handleTabChange} variant="scrollable" scrollButtons="auto" className='mb-8' indicatorColor="secondary">
                                {tabs.map((tab, index) => (

                                    <Tab key={index} label={
                                        <span className={`text-purple-900 ${selectedTab === index?"font-semibold text-lg":""}`}>
